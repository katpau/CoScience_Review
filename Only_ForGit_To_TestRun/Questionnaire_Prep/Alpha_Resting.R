...
TaskName = "Alpha_Resting"


# Scores that are used without change
Scored_Subscales = c("BISBAS_BAS", "RSTPQ_BAS", "BISBAS_BIS", "RSTPQ_BIS", "PSWQ_Concerns", "BFI_Anxiety")
...
PCA_Subscales = c("BISBAS_BAS","RSTPQ_BAS", "BFI_Assertiveness",  "BFI_EnergyLevel", 
                  "MPQPE_SocialPotency",   "MPQPE_Achievement",   "MPQPE_Wellbeing")
PCA_Subscales2 =  c("BISBAS_BIS","RSTPQ_BIS","PSWQ_Concerns", "BFI_Anxiety")
...
Z_Scores_Average$BAS = PCA_Subscales
Z_Scores_Average$BIS = PCA_Subscales2


# Merge all Scales for Indexing
Maha_Subscales = unique(c(Scored_Subscales, Covariates_Subscales, PCA_Subscales, PCA_Subscales2)) # For exclusions, must be non-colinear!
Allrelevant_Subscales = unique(c(Scored_Subscales, Covariates_Subscales, Maha_Subscales,
                                 PCA_Subscales, PCA_Subscales2, unlist(Z_Scores_Sum), unlist(Z_Scores_Average))) # For indexing


# which Factors should be kept from Factor Analysis?
FactorsToKeep = c(1,1,1,1) 
FactorsToKeep2 = c(1,1,1,1) 
