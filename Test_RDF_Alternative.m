...
AnalysisName = "Stroop_LPP"; % name of Analysis, used to structure outputs
ImportedTask = "Stroop"; % name of Task that should be analysed, used to import right files and structure outputs
MatlabAnalysisName = "Stroop_LPP"; % is used in the step functions for specific choices. Usually the same as Analysis Name unless multiple Tasks are included in the Analysis
...
InputFolder=strcat(RootFolder, "Only_ForGit_To_TestRun/Alpha_Context/task-", ImportedTask, "/" );
...
addpath(strcat(RootFolder, "Step_Functions/Quantification_Stroop_LPP"))
...
OutputFolder=strcat(RootFolder, "Only_ForGit_To_TestRun/", AnalysisName, "/task-ChecknewWay/" );

parfor_Forks_OtherAnalysis(IndexSubjectsSubset,      ListFile,  DesignFile, ForkingFile, OutputFolder, ...
    InputFolder, MatlabAnalysisName, MaxStep,    RetryError, LogFolder, ParPools, PrintLocation)
