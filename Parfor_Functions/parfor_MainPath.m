function parfor_MainPath(IndexSubjects, SubjectListFile, DESIGN, OUTPUT, OutputFolder, ImportFolder, AnalysisName, SubsetSize, RetryError, LogFolder, ParPools, PrintLocation)
    ...
    % [ocs] For the main path, the first row of the OUTPUT (i.g., the loaded
    % FORKS.mat) will be selected. However, individual rows of can be processed.

    % Row of the OUTPUT selected for processing.
    OUTPUT_ROW = 1;
    ...
    OUTPUT_Choices = split(OUTPUT(OUTPUT_ROW, 1), '%')';
    ...
    % Prepare Matlabpool
    % [ocs] If a pool is still busy or has a different size, close it
    if ~isempty(gcp('nocreate')) && ...
            (gcp('nocreate').Busy || gcp('nocreate').NumWorkers ~= ParPools)
        delete(gcp('nocreate'));
