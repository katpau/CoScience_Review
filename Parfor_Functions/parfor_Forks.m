function  parfor_Forks(Subject, NotRelevant, DESIGN, OUTPUT, OutputFolder, ImportFolder, AnalysisName, MaxStep, RetryError, LogFolder, ParPools, PrintLocation)
...
fprintf("Display Matlab Input:\n  SUBJECT:%s\n DESIGN:%s\n OUTPUT:%s\n OUTPUTFOLDER:%s\n IMPORTFOLDER:%s\n ANALYSISNAME:%s\n MAXSTEP:%s\n LOGFOLDER:%s\n", ...
    Subject, DESIGN, OUTPUT, OutputFolder, ImportFolder, AnalysisName, MaxStep, LogFolder)
...
% make input numeric
RetryError = str2double(char(RetryError));
ParPools = str2double(char(ParPools));
PrintLocation = str2double(char(PrintLocation));
MaxStep = str2double(char(MaxStep));
...
if contains(AnalysisName, "Resting")
    if contains(AnalysisName, "Context")
        RawEEGFile= dir(ImportFolder+"/"+Subject+"/eeg/*run-2_eeg.set");
    else
        RawEEGFile= dir(ImportFolder+"/"+Subject+"/eeg/*run-1_eeg.set");
    end

else
    RawEEGFile= dir(ImportFolder+"/"+Subject+"/eeg/*_eeg.set");
end
...
Retry_Loop_Max = 0;
while sum(RetryFork)>1 && Retry_Loop_Max < 2
    fprintf(' Number of Combinations that need to be retried/calculated: %d. \n',  sum(RetryFork));
...
fprintf('\n ************************ \n Summary on Achievements for \n  Running took %0.3f hours. \n Newly Completed Paths: %d. \n Previously Completed Paths: %d. \n Encounted Errors: %d. \n \n ************************ \n ',  ...
    ElapsedTime_Hours, CountCompleted,  CountPreviouslyCompleted, CountErrors)
...
for iPath = 1:size(OUTPUT_Choices,1)
    try
    	% For Files
        RunningFileName = NaN;
