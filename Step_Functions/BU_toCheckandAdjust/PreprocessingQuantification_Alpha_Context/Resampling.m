...
try % For Error Handling, all steps are positioned in a try loop to capture errors
    ...
    % This analysis concerns three different Tasks (Gambling, Resting,
    % Picture). Therefore this analysis handles them differently depending
    % on what task is analysed (this is also important for later stages
    % such as epoching). Here it is important that for the Resting state,
    % the SECOND recording is taken.
    if contains(FilePath_to_Import, "task-Resting/")
    ...
    % ****** Make Note ******
    % This analysis includes different Tasks, make note on the task
    % this file is covering.
    if contains(FilePath_to_Import, "task-Resting/")
        EEG.Analysed_Task = "Resting-run_2";
    elseif contains(FilePath_to_Import, "task-Gambling/")
        EEG.Analysed_Task = "Gambling";
    elseif contains(FilePath_to_Import, "task-Stroop/")
        EEG.Analysed_Task = "Stroop";
    end
    ...
    % If file does not exist make note
    if isfile(strcat(FilePath_to_Import_File, "/", File_to_Import))
        ErrorMessage = strcat("File Not Found. //", ErrorMessage);
    end
