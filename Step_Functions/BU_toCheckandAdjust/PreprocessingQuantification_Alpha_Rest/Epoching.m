...
StepName = "Epoching";
Choices = ["2_s_50_overlap", "2_s_90_overlap", "1_s_50_overlap", "1_s_90_overlap" ];
...
try % For Error Handling, all steps are positioned in a try loop to capture errors
    ...
    for i_cond = 1:length(Conditions)
        ...
        % if Resting data was epoched before, make contious again before
        % continueing
            if strcmp(INPUT.StepHistory.Epoching_AC, "epoched")
                ...
                % Remove boundaries between consecutive epochs
                IdxDelete = [];
