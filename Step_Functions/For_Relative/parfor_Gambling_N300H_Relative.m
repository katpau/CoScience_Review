...
parfor iFolder = 1:length(Folders)
    ...
    while is500 == 0
        ...
        INPUT = INPUT.Data;
        if ismember(INPUT.data.srate, [500, 250, 125])
            is500 = 1;
        end
    end
    ...
    for ifile = 1:length(Files_Fork)
        try
            ...
            idx = []; idx=find(contains(Data.Data.data.event, Condition_Triggers(:)));
    ...
    BinStarts_IBI = Data.Data.data.ECGtimes(bin_range(1:length(bin_range)-1) + id_Time);
    ...
    bin_range = round(linspace(1,length(id_Time : id2_Time),no_bins+1));
    BinStarts_EEG = Data.Data.data.EEGtimes(bin_range(1:length(bin_range)-1) + id_Time);
    ...
    for i_Files = 1 : length(Files_Fork)
        ...
        try
            ...
            if strcmp(INPUT.StepHistory.Cluster_Electrodes,"cluster")
                ...
                % elctrode cluster
                NrElectrodes = 1;
                Electrodes = INPUT.data.Electrodes;
             else
                Electrodes = INPUT.data.Electrodes;
            ...
            for i = 1:size(Condition_Triggers,2)
                ...
                % find index of relevant condition triggers within the EEG.event structure
                idx = []; idx=find(contains(INPUT.data.event, Condition_Triggers(:,i)));
            ...
            if INPUT.StepHistory.Cluster_Electrodes == "no_cluster_butAV"
                NrElectrodes = 1;
                Electrodes = append('AV_of: ', join(Electrodes{1}));
                N300H = mean(N300H,1);
            end
            ...
            Lab_L = repmat(INPUT.RecordingLab, NrConditions*NrElectrodes*NrBins,1 );
            Experimenter_L = repmat(INPUT.Experimenter, NrConditions*NrElectrodes*NrBins,1 );
