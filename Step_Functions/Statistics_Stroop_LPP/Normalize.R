Normalize = function(input = NULL, choice = NULL) {
  ...
  # (2) Apply Normalization to Personality Data and Mood Ratings (only one value per Subject!)
  ...
  if (choice != "None") {
    ...
    #########################################################
    # (2) Normalize Personality Variables and Self Reports
    #########################################################
    
    # this is done across subjects and there should be only one value per Subject when normalizing
    Relevant_Collumns =  colnames(output)[grep(c("Personality_|Covariate_|Behav_"), names(output))]
    ...
    # Apply Normalization for RewP and P3 separately
    for (iComponent in c("RewP", "P3")) {
