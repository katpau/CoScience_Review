...
try % For Error Handling, all steps are positioned in a try loop to capture errors
    ...
    if strcmp(Choice , "-200 0")
        ChoiceFMT = "-400 -200";
    elseif strcmp(Choice , "-500 0")
        ChoiceFMT = "-500 0";
    elseif strcmp(Choice , "-100 0")
        ChoiceFMT = "-400 -200";
    end
    ...
    for i_cond = 1:length(Conditions)
        ...
        try
            ...
            Test = str2num(Choice);
            if Test(1) - EEG.xmin*1000 < 5 % Times in s not ms
                EEG = pop_rmbase( EEG, [EEG.xmin*1000 Test(2)] ,[]);
            else
                error('Baseline Window outside of Epoch');
            end
        end
	% Baseline correction of Theta in Quantification step!
