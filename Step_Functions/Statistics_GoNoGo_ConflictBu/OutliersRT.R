OutliersRT = function(input = NULL, choice = NULL) {
  ...
  if (choice == "Applied"){
    ...
    output_RT = output[,c("ID", "Condition_Instruction","Condition_Type", "Behav_RT")]
    output_RT = output_RT[!is.na(output_RT$Behav_RT), ]
    output_RT = output_RT[!duplicated(output_RT), ]
    
    
    outliers_RT = output_RT %>%
      group_by(Condition_Instruction,Condition_Type)%>%
    ...
    output =  merge(    output, 
                        outliers_RT,
                        by = c("ID", "Condition_Instruction","Condition_Type" ),
    ...
    if (Treatment == "Replace" ){
      ...
      output =  merge(output,    
                      MinMax,
                      by = c("Condition_Instruction","Condition_Type"),   
                      all.x = TRUE,    all.y = FALSE )
