Covariate = function(input = NULL, choice = NULL) {
  StepName = "Covariate"
  Choices = c("None", "Gender_MF", "Gender",  "BDI_Depression","BFI_Extraversion","BFI_OpenMindedness","BFI_Conscientiousness","BFI_Agreeableness","BFI_NegativeEmotionality", "Big5_OCEAN")
  Order = 4
  output = input$data
  ...
  # (3) Keep  only the relevant Scores (for IV, DV and Covariates)
  ...
  ## Also take other Scores needed
  PersonalityData = ScoreData[, c("ID", 
                                  paste0("Personality_", c(Personality_Variable_choice, 
                                  "AGG_Anger", "NFC_NeedForCognition", "PTM_Altruism")),
                                  "Covariate_Age")] # Also keep Age for Analysis
  ...
  #########################################################
  # FMT is not forked
  output = output[!output$Component == "FMT",]
  ...
  output = output %>%
    separate(Offer, c("Offer","Response"), "_")
  ...
  FactorVariables = c("ID", "Offer", "Response", "Electrode", 
  ...
  GroupingVariables = c("Offer", "Response", additional_Factors_Name )
