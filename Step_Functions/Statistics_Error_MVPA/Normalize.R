Normalize = function(input = NULL, choice = NULL) {
  ...
  if (choice != "None") {
    ...
    Relevant_Collumns =  colnames(output)[grep(c("Covariate_"), names(output))]
    ...
    #########################################################
    # (3) Normalize EEG data (Onsets) per Task
    #########################################################
    # Get Grouping Variables from previous step
    for (iTask in c("GoNoGo", "Flanker")) {
    ...
    output$EEG_Signal[ output$Task == iTask] = normalize_data(output$EEG_Signal[output$Task == iTask], choice)
    ...
    # (4) Normalize RTs
    #########################################################
    RTs = output[output$Component == "RTs",c("ID", "Behav_RT", "Condition", "Task", "Component")]
    # Remove from output file
    ...
    for (iTask in c("GoNoGo", "Flanker")) {
      ...
      if (any(ChecknotNormal$notNormal)) {
        RTs$Behav_RT[RTs$Task == iTask] = normalize_data(RTs$Behav_RT[ output$Task == iTask], choice)
        
      }}
    
    
    # Merge again with output
    output =  merge(output,  RTs, by = c("ID", "Condition", "Task", "Component"),
                    all.x = TRUE,  all.y = FALSE )
