...
try % For Error Handling, all steps are positioned in a try loop to capture errors
    ...
    for i_cond = 1:length(Conditions)
        ...
        elseif strcmpi(Choice,"RTs_accuracy")
            Correct = cellfun(@(x)~isempty(x) && ismember(x, 1), {EEG.event.ACC});
            Go = cellfun(@(x)~isempty(x) && ismember(x, 'Go'), {EEG.event.Type});
            NoGo = cellfun(@(x)~isempty(x) && ismember(x, 'NoGo'), {EEG.event.Type});
            RT =  cellfun(@(x)~isempty(x) && x > 0.1 && x <0.8, {EEG.event.RT});
            % apply RT cutoff only to Go Trials
            IdxKeepNoGo = and(NoGo, Correct);
            IdxKeepGo = and(Go, Correct); IdxKeepGo = and(IdxKeepGo, RT);
            IdxKeep = or(IdxKeepNoGo, IdxKeepGo);
