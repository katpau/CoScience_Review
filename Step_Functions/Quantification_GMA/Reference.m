function OUTPUT = Reference(INPUT, Choice)
    ...
    try % For Error Handling, all steps are positioned in a try loop to capture errors
        ...
        for i_cond = 1:length(Conditions)
            ...
            % ****** Apply new reference ******
            % [ocs]: CHANGE Only re-reference if the reference channel is different.
            if ~strcmpi(prevRef, "CAV") && strcmpi(Choice, "CAV")
                EEG = pop_reref(EEG, []);
            elseif ~strcmp(prevRef, "Mastoids") && strcmpi(Choice, "Mastoids")
