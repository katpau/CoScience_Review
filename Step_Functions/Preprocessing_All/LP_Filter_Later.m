function  OUTPUT = LP_Filter_Later(INPUT, Choice)
...
StepName = "LP_Filter_Later";
Choices = ["30", "40", "60", "No_LP_Later"];
Conditional = ["LP_Filter_Early == ""No_LP_Early"" ", "LP_Filter_Early == ""No_LP_Early"" ", "LP_Filter_Early == ""No_LP_Early"" ", "LP_Filter_Early ~= ""No_LP_Early"" "];
...
try % For Error Handling, all steps are positioned in a try loop to capture errors
    ...
    for i_cond = 1:length(Conditions)
        ...
        % Apply Filter
        if ~strcmpi(Choice, "No_LP_Later")
            EEG = pop_eegfiltnew(EEG, 'locutoff', [], 'hicutoff', str2double(Choice));
        end
