...
if AnalysisName == "Flanker_MVPA"
    input_dir = [bdir '\Analysis_Functions\Error_MVPA\LRP\preprocessed_data\flanker\'];   %Folder containing the raw data
    output_dir = [bdir '\Analysis_Functions\Error_MVPA\LRP\lrp_onsets\flanker\'];      %Folder where the preprocessed data is to be stored 
 elseif AnalysisName == "GoNoGo_MVPA" 
    input_dir = [bdir '\Analysis_Functions\Error_MVPA\LRP\preprocessed_data\go_nogo\'];   %Folder containing the raw data
    output_dir = [bdir '\Analysis_Functions\Error_MVPA\LRP\lrp_onsets\go_nogo\'];      %Folder where the preprocessed data is to be stored Preprocessed Data
end

%% load preprocessed data
filename = [input_dir, 'data_lrp_aver.mat'];
load(filename);

%% compute lrp onsets
% compute mean amplitude for every time step when id = i is left out
part = unique(data_lrp_aver.id);

for i = 1:length(part)
    remove_part = ismember(data_lrp_aver.id, part(i)); % define participant to leave out
    data_subset = data_lrp_aver(~remove_part,:); % leave participant out
