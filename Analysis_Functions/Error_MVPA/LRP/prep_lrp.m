...
for part = 1:length(participant_codes)
    ...
    if isempty(EEG_data.Data.data.EEG.data_lrp_error)
        ...
        EEG_data.Data.data.EEG.data_lrp_aver(:,2) = ms;
        ...
        EEG_data.Data.data.EEG.data_lrp_aver(:,3) = id;
...
%         for i = 1:length(EEG_data.Data.data.EEG.data_lrp_error(:,1))
%             EEG_data.Data.data.EEG.data_lrp_error_aver(i) = mean(EEG_data.Data.data.EEG.data_lrp_error(i,:));
%         end
%         clear i
%     
%         
...
%            data_lrp_error_aver = EEG_data.Data.data.EEG.data_lrp_error_aver; %create new if it doesn't exist
%         else 
%            if ismember(part_code, data_lrp_error_aver.id) == 0 %only append when current part has not been appended 
%            data_lrp_error_aver = vertcat(data_lrp_error_aver, EEG_data.Data.data.EEG.data_lrp_error_aver); %append if it exists
%            end
%         end
%         
