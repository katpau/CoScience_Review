test_Hypothesis_CK_formula = function (Name_Test,lm_formula, Subset, columns_to_keep, Effect_of_Interest, SaveUseModel, ModelProvided, lmFamily, Nullmodel) {
  ...
  if (SaveUseModel == "exportModel")  {
    ...
    if (is.character(Model_Result) &&  grepl( "Error", Model_Result[1])) {
      ...
      if (lmFamily == "binominal") {
        ...
        coef_model <- summary(Model_Result)$coefficients
        
        # Calculate odds ratios
        odds_ratios <- exp(coef_model[, "Estimate"])
        
        # Confidence intervals for fixed effects
        conf_intervals <- exp(confint(Model_Result, method = "Wald"))
        
        results <- data.frame(
          Term = rownames(coef_model),
          Odds_Ratio = odds_ratios,
          Lower_CI = conf_intervals[4:nrow(conf_intervals), 1],
          Upper_CI = conf_intervals[4:nrow(conf_intervals), 2]
        )
       
        partial_Eta = t(c("Odds_Ratio", 
                          odds_ratios[Idx_Effect_of_Interest+1],
                          conf_intervals[Idx_Effect_of_Interest+4,1],
                          conf_intervals[Idx_Effect_of_Interest+4,2]))
                          
        
        
        # Get p value
        p_Value = NA # check summary p value
      } else {
